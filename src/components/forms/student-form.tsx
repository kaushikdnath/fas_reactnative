...
export function StudentForm({
  ...
  const [batchId, setBatchId] = useState<number | undefined>(initial?.batchId ?? presetBatchId);
  ...
  const [guardianMobile, setGuardianMobile] = useState(initial?.guardianMobile ?? '');
  ...
  useEffect(() => {
    if (!initial && batchId === undefined && batches.length > 0) setBatchId(batches[0].id);
  ...
  const pickPhoto = async (source: 'camera' | 'gallery') => {
    ...
    if (permission.status !== 'granted') {
      setPermissionNote(`${source === 'camera' ? 'Camera' : 'Photo library'} permission was denied. Enable it in system settings.`);
      return;
    }
    try {
      const result =
    ...
    <ThemedView style={styles.page}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.photoSection}>
          <Pressable onPress={() => pickPhoto('camera')} onLongPress={() => pickPhoto('gallery')}>
            {photoUri ? (
          ...
          <View style={styles.photoActions}>
            <Button label="Camera" variant="outlined" onPress={() => pickPhoto('camera')} />
            <Button label="Gallery" variant="outlined" onPress={() => pickPhoto('gallery')} />
          </View>
          {permissionNote ? (
            <ThemedText type="small" themeColor="error" style={styles.permissionNote}>
              {permissionNote}
            </ThemedText>
          ) : null}
        </View>

        <TextField label="Student code" value={code} onChangeText={setCode} placeholder="e.g. STU-0001" autoCapitalize="characters" error={errors.code} />
        <TextField label="Full name" value={name} onChangeText={setName} error={errors.name} />

        <ThemedText type="label" themeColor="textSecondary" style={styles.groupLabel}>BATCH</ThemedText>
        <ChipRow options={batches.map((b) => ({ value: b.id, label: b.name }))} selected={batchId ?? -1} onSelect={setBatchId} />
        {errors.batch ? <ThemedText type="small" themeColor="error" style={styles.batchError}>{errors.batch}</ThemedText> : null}

        <TextField label="Address (optional)" value={address} onChangeText={setAddress} multiline />
        <TextField label="Guardian name" value={guardianName} onChangeText={setGuardianName} error={errors.guardianName} />

        <ThemedText type="label" themeColor="textSecondary" style={styles.groupLabel}>RELATIONSHIP</ThemedText>
        <ChipRow options={RELATIONSHIPS.map((r) => ({ value: r, label: r }))} selected={guardianRelationship} onSelect={setGuardianRelationship} />

        <TextField label="Guardian mobile" value={guardianMobile} onChangeText={setGuardianMobile} keyboardType="phone-pad" error={errors.guardianMobile} />
        <TextField label="Student mobile (optional)" value={studentMobile} onChangeText={setStudentMobile} keyboardType="phone-pad" error={errors.studentMobile} />
        ...
        {submitError ? <ThemedText type="small" themeColor="error" style={styles.submitError}>{submitError}</ThemedText> : null}

        <Button label={initial ? 'Save changes' : 'Add student'} onPress={handleSubmit} loading={submitting} style={styles.submitBtn} />
      </ScrollView>
    </ThemedView>
  );
}
...
const styles = StyleSheet.create({
  ...
  photoSection: { alignItems: 'center', marginBottom: Spacing.four, gap: Spacing.two },
