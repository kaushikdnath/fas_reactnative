...
export default function StudentDetail() {
  ...
  const load = useCallback(async () => {
    ...
    if (s.ok) setStudent(s.value);
    else setError(s.failure.message);
  ...
  const handleMove = async (batchId: number) => {
    ...
    if (result.ok) {
      await logAudit("student.move", { studentId, batchId });
      load();
    }
  };
  ...
  const handleDeleteFingerprint = async (fp: Fingerprint) => {
    ...
    try {
      if (await FingerprintScanner.isConnected())
        await FingerprintScanner.deleteModel(fp.deviceSlot);
    } catch {
      // Sensor may be disconnected; the SQLite row is still removed below.
    }
    const result = await deleteFingerprint(fp.id);
    if (result.ok) {
      await logAudit("fingerprint.delete", { fingerprintId: fp.id, studentId });
      load();
    }
  };
      ...
      <AppBar
        ...
        action={
          <Button
            label="Edit"
            variant="text"
            onPress={() => router.push(`/students/${studentId}/edit`)}
          />
        }
      />
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.badges}>
            ...
            <Badge
              ...
              tone={student.active ? "success" : "warning"}
            />
            <Badge
              label={`${fingerprints.length}/5 fingerprints`}
              tone={fingerprints.length > 0 ? "success" : "neutral"}
            />
          </View>
        </View>
        ...
        <View style={styles.actionsRow}>
          ...
          <Button
            ...
            variant="tonal"
            onPress={() => router.push(`/students/${studentId}/enroll`)}
            style={styles.actionBtn}
          />
        </View>
            ...
            <ListRow
              ...
              trailing={
                <Button
                  label="Remove"
                  variant="text"
                  onPress={() => handleDeleteFingerprint(fp)}
                />
              }
            />
          ))
        ...
        {history.length === 0 ? (
          ...
          history.map((h) => (
            <ListRow
              key={h.id}
              title={h.attendanceDate}
              subtitle={`In ${fmtTime(h.inTime)} \u00B7 Out ${fmtTime(h.outTime)}`}
            />
          ))
        )}
      ...
      >
        <View style={styles.backdrop}>
          <ThemedView type="surface" style={styles.moveCard}>
            <ThemedText type="subtitle" style={styles.moveTitle}>
              Move to batch
            </ThemedText>
            <FlatList
