...
import { listFingerprintsForStudent, saveFingerprint } from '@/data/fingerprint-repository';
...
export default function EnrollFingerprint() {
  ...
  useEffect(() => {
    ...
    listFingerprintsForStudent(studentId).then((r) => { if (r.ok) setExisting(r.value); });
    FingerprintScanner.isConnected().then(setConnected).catch(() => setConnected(false));
  }, [studentId]);

  useEffect(() => {
    if (!FingerprintScanner.events) return;
    const sub = FingerprintScanner.events.addListener(AS608_EVENTS.enroll, (e: { message: string }) => {
      setStatusText(e.message);
    });
    return () => sub.remove();
  }, []);

  const availableFingerNames = FINGER_NAMES.filter((f) => !existing.some((e) => e.fingerName === f));
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      if (slot === undefined) throw new Error('The sensor has no free slots. Clear unused templates first.');
      ...
      setStage('error');
      setErrorText(e instanceof Error ? e.message : 'Enrollment failed -- please try again');
    }
  };
  ...
  if (!student) {
    return (
      <ThemedView style={styles.page}>
        <AppBar title="Enroll fingerprint" onBack={goBack} />
        <LoadingView />
      </ThemedView>
    );
  }
  ...
  if (atCapacity) {
    return (
      <ThemedView style={styles.page}>
        <AppBar title="Enroll fingerprint" onBack={goBack} />
        <EmptyStateView icon="\u2705" title="All 5 slots enrolled" subtitle={`${student.name} already has the maximum of ${MAX_FINGERPRINTS_PER_STUDENT} fingerprints.`} />
      </ThemedView>
    );
  }
  ...
  return (
    <ThemedView style={styles.page}>
      <AppBar title="Enroll fingerprint" subtitle={student.name} onBack={goBack} />
      <View style={styles.content}>
        <ThemedText type="label" themeColor="textSecondary" style={styles.label}>WHICH FINGER?</ThemedText>
        <ChipRow options={availableFingerNames.map((f) => ({ value: f, label: f }))} selected={fingerName} onSelect={setFingerName} />
        ...
        <View style={styles.statusCard}>
          ...
          <ThemedText type="body" themeColor="textSecondary" style={styles.statusBody}>
            {stage === 'error' ? errorText : statusText || 'The sensor will ask for the same finger twice to build a reliable template.'}
          </ThemedText>
        </View>

        <Button
          label={stage === 'enrolling' || stage === 'uploading' ? 'Enrolling\u2026' : 'Start enrollment'}
