...
export default function Device() {
  ...
  const refreshStatus = useCallback(async () => {
    const isConnected = await FingerprintScanner.isConnected().catch(
      () => false,
    );
  ...
  useEffect(() => {
    FingerprintScanner.listDevices()
      .then(setDevices)
      .catch(() => setDevices([]));
    refreshStatus();
  }, [refreshStatus]);
  ...
  return (
    <ThemedView>
      ...
      <View style={styles.statusRow}>
        ...
        {connected ? (
          <Badge label={`${freeSlots ?? "\u2014"} free slots`} tone="neutral" />
        ) : null}
      </View>
        ...
        <>
          ...
          {devices.length === 0 ? (
            <EmptyStateView
              ...
              subtitle="Plug in the CH340/AS608 adapter, then pull down to refresh."
            />
          ) : (
            <FlatList
              data={devices}
      ...
      >
        <ThemedText type="body">
          This erases every fingerprint template stored on the physical sensor.
          Templates saved on this phone are not affected and can be re-uploaded
          to the sensor afterward.
        </ThemedText>
      </ConfirmDialog>
    </ThemedView>
  );
