...
export async function listBatches(includeInactive = true): Promise<Result<Batch[]>> {
  try {
    const d = await getDb();
    const rows = await d.getAllAsync<BatchRow>(
...
export async function getBatch(id: number): Promise<Result<Batch>> {
  try {
    const d = await getDb();
    const row = await d.getFirstAsync<BatchRow>(
      `SELECT b.*, (SELECT COUNT(*) FROM students s WHERE s.batch_id = b.id AND s.active = 1) student_count
...
export async function createBatch(draft: BatchDraft): Promise<Result<Batch>> {
  try {
    ...
    const r = await d.runAsync(
      ...
      draft.name, draft.academicYear, draft.active === false ? 0 : 1, now(),
    );
...
export async function updateBatch(id: number, draft: BatchDraft): Promise<Result<Batch>> {
  try {
    const d = await getDb();
    await d.runAsync(
      'UPDATE batches SET name = ?, academic_year = ?, active = ? WHERE id = ?',
      draft.name, draft.academicYear, draft.active === false ? 0 : 1, id,
    );
    return getBatch(id);
  } catch (e) {
...
export async function deleteBatch(id: number, options: BatchDeleteOptions = {}): Promise<Result<void>> {
  try {
    const d = await getDb();
    await d.withTransactionAsync(async () => {
      if (options.cascadeStudents) {
        const students = await d.getAllAsync<{ id: number }>('SELECT id FROM students WHERE batch_id = ?', id);
        const ids = students.map((s) => s.id);
        if (ids.length > 0) {
          const placeholders = ids.map(() => '?').join(',');
          if (options.cascadeFingerprints) {
            await d.runAsync(`DELETE FROM fingerprints WHERE student_id IN (${placeholders})`, ...ids);
          }
          if (options.cascadeAttendance) {
            await d.runAsync(`DELETE FROM attendance WHERE student_id IN (${placeholders})`, ...ids);
          }
    ...
    return Err('Could not delete batch -- move or remove its students first', e);
  }
}
